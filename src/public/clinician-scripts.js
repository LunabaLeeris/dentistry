...
buttons.forEach(button => {
    button.addEventListener('click', function() {
        ...
        map_route = { // DEFAULT VALUES
            ...
            'procedure-logs':  `/clinician/procedure-logs/0/procedure_report_index/desc`,
            'patient-chart' : `/clinician/patient-chart/0/patient_last_updated/desc`,
            'grades' : `/clinician/grades`,
            'tally-sheets' : `/clinician/tally-sheets`,
            'cor' : `/clinician/cor`
        }
...
document.getElementById('table-data').addEventListener('click', function(event) {
    ...
    const map_view = {
        ...
        'back-to-proced-logs'   : `/clinician/procedure-logs/0/procedure_report_index/desc/${undefined}/0`,
    ...
    const map_add = { // POST REQUESTS
        ...
        'search'             : search,
        'edit-procedure-log' : edit_procedure_log
    };

    const actions = { // RE-ROUTING
        'lab-view'     : `/clinician/view-laboratory-log/`,
        ...
        'proced-edit'  : `/clinician/edit-procedure-log/`
    }
...
function edit_procedure_log(event){
    event.preventDefault();
    ...
    var patientSelect = document.getElementById('patient-name');
    var patientValue = patientSelect.value;
    
    if (patientValue == 'Select instructor') { 
        patientSelect.focus();
        return;
    }

   const myForm = document.getElementById('ProcedureForm');
   if (!myForm.checkValidity()){
       Array.from(myForm.elements).forEach(element => {
           if (element.hasAttribute('required')) {
               element.reportValidity();
   ...
   const data = new FormData(myForm);
    ...
    console.log(event.target.name);
   // handle fetching logic
   fetch(`/clinician/edit-procedure-log/${event.target.name}`, {
        method: 'POST',
        headers: {
                'Content-Type': 'application/json'
            },
   ...
   .then(htmlResponse => {
       // Update the innerHTML of an element with the received HTML response
       document.getElementById('table-data').innerHTML = htmlResponse;
       open_alert("edit-procedure-success", "success", "Procedure Log succesfully edited!");
   })
   .catch(error => {
       console.error('Error:', error);
   });
